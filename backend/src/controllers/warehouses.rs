...
) -> Result<Json<ApiResponse<Vec<WarehouseResponse>>>, (StatusCode, Json<ApiResponse<()>>)> {
...
pub async fn create(
...
) -> Result<Json<ApiResponse<WarehouseResponse>>, (StatusCode, Json<ApiResponse<()>>)> {
    ...
    Ok(Json(ApiResponse::ok(WarehouseResponse {
        id: w.id,
        name: w.name,
        created_at: w.created_at.to_rfc3339(),
    })))
...
pub async fn update(
    ...
    let w = warehouse::Entity::find_by_id(id)
        .filter(warehouse::Column::DeletedAt.is_null())
        .one(&state.db)
        .await
        .map_err(|e| {
    ...
    let mut a: warehouse::ActiveModel = w.into();
