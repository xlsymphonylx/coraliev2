...
use crate::{
    dto::{
        common::ApiResponse,
        product_image::{CreateImageRequest, ImageResponse, UpdateImageRequest},
    },
...
) -> Result<Json<ApiResponse<Vec<ImageResponse>>>, (StatusCode, Json<ApiResponse<()>>)> {
...
pub async fn create(
...
) -> Result<Json<ApiResponse<ImageResponse>>, (StatusCode, Json<ApiResponse<()>>)> {
    let i = product_image::ActiveModel {
    ...
    Ok(Json(ApiResponse::ok(ImageResponse {
        id: i.id,
        product_id: i.product_id,
        url: i.url,
        alt: i.alt,
...
pub async fn update(
    ...
    let img = product_image::Entity::find_by_id(id)
        .filter(product_image::Column::DeletedAt.is_null())
        .one(&state.db)
        .await
        .map_err(|e| {
