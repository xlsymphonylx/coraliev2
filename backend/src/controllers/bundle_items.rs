...
) -> Result<Json<ApiResponse<Vec<BundleItemResponse>>>, (StatusCode, Json<ApiResponse<()>>)> {
    Ok(Json(ApiResponse::ok(
        bundle_item::Entity::find()
            .filter(bundle_item::Column::DeletedAt.is_null())
            .all(&state.db)
            .await
            .map_err(|e| {
                (
...
pub async fn create(
    State(state): State<AppState>,
    Json(body): Json<CreateBundleItemRequest>,
) -> Result<Json<ApiResponse<BundleItemResponse>>, (StatusCode, Json<ApiResponse<()>>)> {
    let b = bundle_item::ActiveModel {
        bundle_id: Set(body.bundle_id),
        product_id: Set(body.product_id),
        quantity: Set(body.quantity.unwrap_or(1)),
    ...
    Ok(Json(ApiResponse::ok(BundleItemResponse {
        id: b.id,
        bundle_id: b.bundle_id,
        product_id: b.product_id,
        quantity: b.quantity,
...
pub async fn update(
    State(state): State<AppState>,
    Path(id): Path<i32>,
    Json(body): Json<UpdateBundleItemRequest>,
) -> Result<Json<ApiResponse<BundleItemResponse>>, (StatusCode, Json<ApiResponse<()>>)> {
    let bi = bundle_item::Entity::find_by_id(id)
        .filter(bundle_item::Column::DeletedAt.is_null())
        .one(&state.db)
        .await
        .map_err(|e| {
